...
private:
    ...
    // 滑动平均滤波器缓存（用于抹平高频抖动）
    std::vector<double> tangent_history_;
    const size_t FILTER_WINDOW_SIZE = 5; // 滤波窗口大小，5帧大约是0.5秒，数值越大越平滑但延迟越高

    // 迟滞滤波器状态（用于解决边界横跳）
    bool is_window_active_ = false; // 当前是否处于有效追踪状态
...
public:
    ...
    void scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
        ...
        if (roi_points.size() >= 5) {
            ...
            // ----------------------------------------------------
            // 算法屏障一：滑动平均滤波（解决树叶带来的微观抖动）
            // ----------------------------------------------------
            tangent_history_.push_back(tangent_angle);
            if (tangent_history_.size() > FILTER_WINDOW_SIZE) {
                tangent_history_.erase(tangent_history_.begin()); // 弹出最老的一帧
            }
            
            // 求队列平均值
            double smoothed_tangent_sum = 0.0;
            for (double val : tangent_history_) {
                smoothed_tangent_sum += val;
            }
            double tangent_angle = smoothed_tangent_sum / tangent_history_.size();
            double tangent_deg = tangent_angle * 180.0 / M_PI;

            // ----------------------------------------------------
            // 算法屏障二：迟滞滤波器（解决临界区骤变横跳）
            // ----------------------------------------------------
            // 定义硬边界
            const double LOWER_LIMIT = -80.0;
            const double UPPER_LIMIT = -20.0;
            const double BUFFER = 4.0; // 4度的缓冲区
            if (is_window_active_) {
                // 如果当前处于“正常追踪”状态，放宽退出门槛
                if (tangent_deg > (UPPER_LIMIT + BUFFER) || tangent_deg < (LOWER_LIMIT - BUFFER)) {
                    is_window_active_ = false; // 真正越界了，关闭追踪
                }
            } else {
                // 如果当前处于“归零”状态，提高进入门槛，防止噪点误触
                if (tangent_deg <= (UPPER_LIMIT - BUFFER) && tangent_deg >= (LOWER_LIMIT + BUFFER)) {
                    is_window_active_ = true; // 稳定进入区间，开启追踪
                }
            }
